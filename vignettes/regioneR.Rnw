...
\title{regioneR: Association analysis of genomic regions based on permutation tests}
\author{Bernat Gel\footnote{bgel@imppc.org}, Anna D{\'i}ez-Villanueva and Roberto Malinverni}
\date{Edited: September 7, 2015; Compiled: \today}
...
In addition, \regioneR{} includes a set of helper functions based on Bioconductor's \Biocpkg{GenomicRanges} infrastructure to manage and manipulate region sets with a simple and consistent interface. 
...
One of the most common uses of \regioneR{} is to answer the question: Do the regions in set A overlap with the regions in B more than expected? For example: Are my ChIP-seq peaks on the promoters of active genes? or Are the break points in repetitive regions? To answer question like these we can use the convenience \Rcode{overlapPermTest} function in regioneR, that evaluates the number of overlaps between two sets of regions. 

To do that, we need to have the two region sets either as a \Rcode{GRanges}, a \Rcode{data.frame} or a bed-like file. An practical way to create a \Rcode{GRanges} is to use the \Rcode{toGRanges} function. In this case, as an example, we will create two sets of random regions with about 50\% overlap.
...
Once we have the two region sets, we can test if A overlaps with B more than expected. To do that we will use \Rcode{overlapPermTest}.
...
We can see that the test was significant, with a p-value \textless0.05. Now, to plot it, we can simply plot the \Rcode{pt} object.
...
In addition, we can test if the association between the two region sets is highly dependant on their exact position. To do that, we can use the \Rcode{localZScore} function.
...
Which in this case shows that moving the regions in A produces a drop in the z-scores and so, shows that the association is dependant on the exact position of the regions and is not a regional effect. More information about the \Rcode{localZScore} function can be found at section \ref{sec:localzscore}: Local Z-score.

In this small example we have used \Rcode{overlapPermTest} to test the number of overlaps between two region sets. Using the more general \Rcode{permTest} function, we can test other types of association using different evaluation functions (the distance, the evaluation of a function over the genomes or custom user-defined functions) and applying different types of randomizations. In addition, this example assumes that the data refers to the human genome and uses a default mask with regions not available for the randomization. In the next sections, all these options are explained in detail.
...
The core functionality of regioneR is to statistically evaluate the association between different RS or between a RS and other genomic features using a permutation test approach. This functionality is supported by the \textit{permTest} function, a parallel and highly customizable function performing the permutation tests and producing the statistical evaluation of the results. 

With \textit{permTest} and the right biological data it is possible to answer different types of biological questions such as:

\begin{itemize}
\item Do my regions contain more SNPs than expected by chance?
\item Are my regions enriched in repetitive regions?
\item Are my regions significantly closer to genes?
\item Are DNA methylation levels in my RS higher than expected?
\item Do my regions overlap with recurrent SCNAs in my samples?
\item Are my ChIP peaks associated with that other histone mark?
\item Do my ChIP regions associate with repressed genes?
\end{itemize}

\textbf{NOTE:} It is important to take into account, however, that regioneR's permutation tests can only test the association between a set of regions and some other feature but not identify which regions contribute the most to that association. Therefore, for questions of the type Identify the region in my RS that associate with something, regioneR is not a suitable choice, and more specific analysis tools should be used. 





\subsection{How Does a Permutation Test Work?}
...
First of all we need to have our RSs loaded into R. To do that we can use the \textit{toGRanges} function.
...
Next thing we need is an evaluation function. In our example we want to test the overlap of our RS with the altered regions, so we will use the \textit{numOverlaps} function, which, given two RS returns the number of overlaps between them.

Using \textit{numOverlaps} we can count the number of overlaps between the two RS, in our case we can compute the number of special genes overlapping an altered region.
...
The least restricted function included in \regioneR{} is \textit{randomizeRegions}, that given a RS, a genome and an optional mask, returns a new RS with the same number of regions and of the same width as the original ones but randomly placed along the non-masked parts of the genome. This is also the slowest of the randomization functions available. 

In our example, since the special genes are a subset of the bigger set of all genes it is much better to use \textit{resampleRegions}, that given a universe of regions, randomly selects a subset of them to create the randomized region sets.
...
\subsection{How to perform a permutation test with \regioneR{}}
The main function to perform a permutation test with \regioneR{} is \Rfunction{permTest}, a function taking a a region set (RS) in any of the accepted formats (see section 4 - Region Sets), a randomization function and an evaluation function and returning a \Rclass{permTestResults} object.
...
In addition to the 3 required parameters, \textit{permTest} accepts other fixed parameters -\textit{ntimes} to specify the number of randomizations, \textit{verbose} to toggle the drawing of a progress bar, \textit{force.parallel} to force or forbid the use of multiple cores to run the analysis...- and it also accepts any additional parameter required by the randomization function (usually a genome and a mask) or the evaluation function.
...
Following our example before with my special genes and the altered regions we could call \textit{permTest} like this: 
...
\textbf{NOTE:} Since \textit{permTest} uses the ellipsis operator (...) to forward the required additional parameters to the evaluation and randomization functions it is strongly recommended to always use named parameters (e.g. A=RS1 instead of only RS1). Failing to do that can result in hard to debug errors.

In any case we would get a \textit{permTestResults} object with the results of the analysis. To view this result we can just print it or use summary. In this case we can see that the association is statistically significant and so we can conclude that the special genes are associated with my altered regions.
...
\subsection{A note on the number of permutations}
...
\incfig[h]{figures/special_genes_5000perm.pdf}{0.8\textwidth}{Significant permutation test}
{This is the plot of a significant permutation test with 5000 permutations.}

%%\begin{center}\vspace{1cm}
%%\includegraphics{./figures/special_genes_5000perm.pdf}
%%%\captionof{figure}{Example of region sets}
...
\incfig[h]{figures/regular_genes_5000perm.pdf}{0.8\textwidth}{Non significant permutation test}
{This is the plot of a non significant permutation test with 5000 permutations.}


%%\begin{center}
%%\includegraphics{./figures/regular_genes_5000perm.pdf}
%%%\captionof{figure}{Example of region sets}
%%\end{center}\vspace{1cm}
...
In \regioneR{}, a randomization function is any function that takes a RS as an argument and returns a RS with the randomized regions. Different randomization functions are included in the package, but it is also possible to create custom randomization functions for more specific needs (see section 4 - Region Sets).

\subsubsection{randomizeRegions}
...
\begin{itemize}
\item \textit{non.overlapping} (default=FALSE): by default, two randomized regions can overlap. Setting non.overlapping to TRUE will force all randomized regions to be non-overlapping. Creating non-overlapping regions is slower.
\item \textit{per.chromosome} (default=FALSE):  if this option is set to true, the chromosome is not randomized, but only the position within it. This option can be useful when there original data is intrinsically biased towards some chromosomes or there is an association between the regions and the crhomosomes.
\end{itemize}
...
The evaluation functions are the functions that define what is being tested in the permutation test. Some evaluation functions are provided in \regioneR{} covering the most common uses but it is possible (and easy) to create your own custom evaluation functions (see section 2.6.1). In general, an evaluation function is simply a function taking a RS and returning a single numeric value.


\subsubsection{numOverlaps}
...
\subsubsection{Custom Randomization}
...
\subsection{A note on reproducibility}
  In order to create reproducible code with functions that use random numbers such as the permutation testing in \regioneR{}, it is necessary to use \textit{set.seed}. However, since regioneR uses \textit{parallel} to perform the test it is also necessary to set the \textit{mc.set.seed} parameter to FALSE to ensure reproducibility.
...
\section{Local Z-score}
\label{sec:localzscore}
...
Internally, \regioneR{} uses \Rclass{GRanges} to represent a RS, and all functions returning an RS return a \Rclass{GRanges} object. However, \regioneR{} accepts different data formats as input thanks to its \Rfunction{toGRanges} function. In particular, it accepts:

\begin{itemize}
\item \textbf{GRanges:} A \textit{GRanges} object from the \textit{GenomicRanges} Bioconductor package.
\item \textbf{Data Frames:} A \textit{data.frame} or any other class inheriting from it with a "BED-like" structure with the first three columns representing chromosome, start and end. There are a few restrictions regarding the naming of the columns in the data frame which are detailed in the \textit{toGRanges} function help page.
\item \textbf{A file:} a file name or connection to a file in any of the formats accepted by \textit{rtracklayer}'s import function (bed, gff, wigg, ...)
\end{itemize}

Two utility functions are provided in \regioneR{} to manage the transformation to and from \textit{GRanges}: \textit{toGRanges} and \textit{toDataframe}.

\begin{itemize}
\item \textbf{\textit{toGRanges}} accepts a RS in any of the supported formats and creates a \textit{GRanges} object from it
\item \textbf{\textit{toDataframe}} accepts a \textit{GRanges} object and transforms it into a \textit{data.frame}
\end{itemize}

For example, we can create a \textit{GRanges} from a \textit{data.frame} representing a RS with 3 regions in chromosome 1  with 2 additional values (x and y) using \textit{toGRanges}:
...
and we can get back to a \textit{data.frame} structure using the \textit{toDataframe} function.

<<>>=
toDataframe(B) 
@

%%Take into account, however, that the column names might not conserved for the part of the data frame representing the chromosomes, starts and ends.

%%<<>>=
%%A <- data.frame(Chromosome.Name=1, Start.Base=c(1,15,24), End.Base=c(10,20,30),
%%  x=c(1,2,3), y=c("a","b","c"))
%%names(A)
%%names(toDataframe(toGRanges(A)))
%%@
...
\regioneR{} provides a function to get genomes and masks, \Rfunction{getGenomeAndMask}, but usually the user won't need to use it directly. In regioneR, all functions accepting a genome and a mask as arguments use this function internally and so, accept any genome and mask specification accepted by these function.

\subsection{Genomes}
In \regioneR{} a genome is simply a list of chromosomes and their length. Therefore, it is possible to specify a genome by giving a \textit{data.frame} with 2 columns (\textit{chr} and \textit{length}) or 3 columns (\textit{chr}, \textit{start} and \textit{end}) or a \textit{GRanges} object with one region per chromosome. However, when working with a standard chromosome available as a \Rclass{BSgenome}, it is possible and much more convenient to rely on the automatic loading of the genome (given that is installed in the system). By specifying the \Rclass{BSgenome} name of the genome -e.g. hg19, mm9, dm2 ...- regioneR will fetch the correct genome information. 


\subsection{Masks}
As with genomes, it is possible to specify a mask as a \textit{data.frame} or \textit{GRanges} object with the "forbidden" regions. However, it is also possible and more convenient to use the automatic retrieving of the default mask from \textit{BSgenome} if it's suitable for the application. Again, to automatically retrieve the mask, the masked version of the \textit{BSgenome} must be installed.

\subsection{How to retrieve a genome and mask}
The \textit{getGenomeAndMask} accepts two parameters, a genome and an optional mask. In general, the genome will be a genome assembly name ('hg19', 'mm10', etc...) and the function will check if there is a genome with that name installed and return it. If none is available, it will prompt the user to install it. If the masked version of the genome is available, that is the one that will be used. If a genome is already available in the form of a \textit{data.frame} or \textit{GRanges} object (or any other valid RS format), the function will return it as a \textit{GRanges}.

The second parameter, \textit{mask}, can be used to specify a mask or to explicitly request no to retrieve one. In particular, if \textit{mask} is NULL (it's default value), it will try to retrieve a mask from the \textit{BSGenome} package, if \textit{mask} is NA it will return an empty mask and if \textit{mask} is anything accepted by \textit{toGRanges} it will return it as the mask.

The function returns a \textit{list} with a genome and a mask elements, both of them \textit{GRanges} objects.


\subsection{Filtering Chromosomes}
The latest versions of some genomes include a number of additional "chromosomes" representing different alternative assemblies and patches for specific complex regions. While these additional chromosomes are useful in some cases, in others they interfere with the analysis, and so it's best to remove them from the genome. To do that and to generally filter any subset of chromosomes, \regioneR{} includes the \textit{filterChomosomes} function. It take a RS and returns a filtered version of it containing only the regions in the specified chromosomes. To use it one need to specify the RS, the organism and the type of chromosome to keep (autosomal, canonical, ...). In addition it is possible to specify a custom list of chromosome names to keep.

The available chromosome types are dependant on the organism, and the \textit{listChrTypes} function can be used to see what organisms and chromosome subsets are available.
...
%
%==============================================================================
\section{Region Set Helper Functions}
\regioneR{} includes a set of helper functions based on the \Biocpkg{GenomicRanges} infrastructure to manage and manipulate RS: \textit{joinRegions}, \textit{commonRegions}, \textit{mergeRegions}, \textit{overlapRegions}, etc... All these functions share a standard and simple signature, with one or two RS in any of the accepted formats and almost all of them (except \textit{splitRegions} and \textit{overlapRegions}) return a \textit{GRanges} object. 



\subsection{Functions operating on a single RS}

\begin{itemize}
\item{\textbf{\textit{extendRegions:}}} This function takes a RS and two integers and extends (enlarges) the regions by the specified amount. If a genome is given, it takes into account the chromosome lengths and does not extend the regions over the chromosome limits. It can take negative extension values to shrink the regions instead of enlarging them.
\item{\textbf{\textit{joinRegions:}}} This function takes a RS and one integer, \textit{min.dist}, and joins the regions that are less than \textit{min.dist} bases apart. 
\end{itemize}

\subsection{Functions operating on two RS}

\begin{itemize}
\item{\textbf{\textit{commonRegions:}}} Given two RS A and B this functions returns a \textit{GRanges} object with their intersection, the genomic regions common to both of them. 
\item{\textbf{\textit{mergeRegions:}}} Given two RS A and B this functions returns a \textit{GRanges} object with the regions of the genome contained in either of them, it is equivalent to merging the to RS into one and then fusing the overlapping regions.
\item{\textbf{\textit{subtractRegions:}}} Given two RS A and B this functions returns a \textit{GRanges} object with the regions in A not present in B, it is, the regions in A minus the parts overlapping those in B.
\item{\textbf{\textit{uniqueRegions:}}} Given two RS A and B this functions return a \textit{GRanges} object with the regions of the genome covered by A or B but not both.
\end{itemize}

\subsection{Other Functions: functions not returning a \textit{GRanges} object}

\begin{itemize}
\item{\textbf{\textit{overlapRegions:}}} \textit{overlapRegions} is one of most powerful and useful RS management functions in \regioneR{}. It has a number of parameters and types of returned values. For a thorough description, refer to the regioneR user guide.

Given two RS A and B, \textit{overlapRegions} identifies the regions in A overlapping a region in B, filters them and returns them. It is possible to filter by the relation between the regions (e.g. "the region from A must be included in the one from B") or by the amount of overlap (e.g. "at least 10 bases", "at least half the region in A"). The returned value might be a complete table with information, for every overlap, of the region in A, the region in B and their relation, a boolean vector indicating for every region in A whether it overlaps any region in B following the filtering criteria or simply the number of valid overlaps between the two RS. In addition, it is possible to specify if the additional data columns associated with the RS have to be kept or might just be ignored.

\textbf{Example:} Given the two same RS each with 10 regions in chromosome 1 we have been using so far we can get the complete table with their overlaps:
...
\section{Usage Examples}
  ...
  In this section we will present a series of examples of how to use \regioneR{} to answer real biological questions.
  
  \subsection{Example 1: CpG Islands and Gene Promoters. A basic example.}
    
    \textbf{\Large{Question: Are Cpg islands and promoters associated?}}
    
    \textbf{\large{Brief Problem Description:}}
    ...
    \textbf{\large{Datasets:}}
      We will use the CpG islands described in Wu et al. 2010 (http://rafalab.dfci.harvard.edu/) and the promoters from UCSC defined as -2000 to +200 base pairs to the TSS according to the genome assembly hg19. The promoters file can be dowloaded from \url{http://gattaca.imppc.org/regioner/}.

      <<eval=FALSE>>=
        #NOT RUN
        set.seed(12345) 
        cpgHMM <- toGRanges("http://rafalab.jhsph.edu/CGI/model-based-cpg-islands-hg19.txt")
        promoters <- toGRanges("http://gattaca.imppc.org/regioner/data/UCSC.promoters.hg19.bed")
      @
    
    \textbf{\large{Step-by-step analysis:}}
      
      The idea of the test is to randomly move the CpG islands along the genome and count how many of them overlap with a promoter. Since we want to test whether two sets of regions have an overlap higher than expected and we don't need a special randomization strategy, we can use the convenience function \textit{overlapPermTest}, that uses \textit{permTest} under the hood, but presets some parameters for us (in particular, it uses \textit{numOverlaps} as the evaluation function and \textit{randomizeRegions} as the randomization strategy).  
      ...
      To ensure that we will count a CpG island only once even if it overlaps 2 or more promoters, we use the parameter \textit{count.once=TRUE}, that will be automatically passed to the \textit{numOverlaps} function. In general, any parameter needed by the randomization or evaluation function can be given here and will be used by the relevant function. 
    ...
    The p-value obtained (>0.001) shows us a strong association. 614 out of 2000 CpG islands overlap at least one promoter while a mean of only 79.087 islands overlapped a promoter in the randomized region sets. These results can be summarised in a single plot by plotting the \textit{perTestResults}.
   ...
   \incfig[h!]{figures/Example1_fig1.pdf}{0.8\textwidth}{Association between CpG islands and gene promoters}
{}
  ...
  \subsection{Example 2: Analysis of ChIP-seq peaks. Advanced usage and local Z-score.}
  
     \textbf{\Large{Question: In the cell line Hepg2 (Human hepatocellular carcinoma ), is Rad21 (Cohesin) associated with CTCF? And with gene promoters? Are these associations different?}}
    ...
    \textbf{\large{Datasets:}}
      ...
      The Rad21 and CTCF ChIP-seq peaks are taken from the ENCODE project (http://www.genome.gov/encode/). The peak files can be downloaded from the UCSC and read using the \Rfunction{toGRanges} function. The promoters will be the same as in Example 1, that is, -2000/+200 around the UCSC TSS.
...
@
    ...
    @
        ...
        To test the associations between Rad21 and CTCF or promoters we will use the \Rfunction{permTest} function. We want to test if the region sets overlap more than expected, so we will use \Rfunction{numOverlaps} as the evaluation function, passing it the \Rcode{count.once} parameter so each region in A is counted at most once. As the randomization function we will use \Rfunction{circularRandomizeRegions}, since it is faster than \Rfunction{randomizeRegions}, will keep the internal structure of the region set (won't change the distance between adjacent regions) and we are not testing against such distances. With that function it might not be possible to ensure a perfect exclusion of the masked regions, but in this case we can tolerate that too. In general, in many situations, \Rfunction{circularRandomizeRegions} is a suitable replacement to \Rfunction{randomizeRegions} with the benefit of being much faster. We will also set \Rcode{mc.cores=4} so 4 processor cores are used and \Rcode{mc.set.seed=FALSE} to ensure a reproducible result. These two parameters not generally needed. 
        
        \textbf{IMPORTANT NOTE:} It is very important to pass the name the parameters passed to \Rcode{permTest}, since it makes use of the ellipsis operator \Rcode{...} and might produce unexpected errors if arguments are passed without a name.
        
    <<eval=FALSE>>=
        #NOT RUN
        pt_Rad21_5k_vs_Ctcf <- permTest(A=HepG2_Rad21_5K, B=HepG2_Ctcf, ntimes=1000, randomize.function=circularRandomizeRegions, evaluate.function=numOverlaps, count.once=TRUE, genome="hg19", mc.set.seed=FALSE, mc.cores=4)

        pt_Rad21_5k_vs_Prom <- permTest(A=HepG2_Rad21_5K, B=promoters, ntimes=1000, randomize.function=circularRandomizeRegions, evaluate.function=numOverlaps, count.once=TRUE, genome="hg19", mc.set.seed=FALSE, mc.cores=4)
        ...
        plot(pt.obj.Rad21_5K.Prom, main="Rad21_5K vs Promoters")
    @
    
    \incfig[h!]{figures/Example2_fig1.pdf}{0.8\textwidth}{Association of Rad21 peaks  with CTCF and gene promoters.}
{}
...
{}
  ...
  Increasing the window value to 10000bp and the step to 200bp, we can see a broader view of the z-score change and we can see how there's a drop of z-score, but at a much higher distance, which indicates either wider regions or some kind of regional association.
    ...
    <<eval=FALSE>>=
        #NOT RUN
        lz_Rad21_vs_Ctcf_2 <- localZScore(A=HepG2_Rad21_5K, B=HepG2_Ctcf, pt=pt_Rad21_5k_vs_Ctcf, window=10000, step=500, count.once=TRUE)

        lz_Rad21_vs_Prom_2 <- localZScore(A=HepG2_Rad21_5K, B=promoters, pt=pt_Rad21_5k_vs_Prom, window=10000, step=500, count.once=TRUE)     
        
        plot(lz_Rad21_vs_Ctcf_2, main="Rad21_5k vs CTCF (10Kbp)")
        plot(lz_Rad21_vs_Prom_2, main="Rad21_5k vs promoters  (10Kbp)")
    @

      \incfig[h!]{figures/Example2_fig3.pdf}{0.8\textwidth}{Local Z-score analysis of the association of Rad21 peaks with CTCF and gene promoters with a 10Kbp window.}
{}

    \textbf{Note:} In many cases, it is possible to use a subset of the regions to get a first glimpse of the association before testing with the whole set of regions. However, the size of the subset is important. In the figure \ref{fig:subset_size} there are the results of testing the association of Rad21 with CTCF and the gene promoters with three different subset sizes: 200, 5000 and all the Rad21 reagions. It is clear that the 5000 subset is capable of detecting the association with both sets of regions but the 200 subset detects the very strong association with CTCF but fails to clearly detect the association with the promoters. 
    
    \incfig[h!]{figures/Example2_fig4.pdf}{0.8\textwidth}{The effect of the subset size in the association testing.}
{}




  \subsection{Example 3: Regions with Copy Number Alterations. Custom evaluation functions.}
