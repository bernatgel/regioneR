...
plot.permTestResults<-function(x, pvalthres=0.05, plotType="Area", ...){
  ...
  if(!hasArg(xlab)) xlab <- paste0(x$evaluate.function.name)
  if(!hasArg(ylab)) ylab <- ""
  if(!hasArg(main)) main <- ""
  ...
  if(is.finite(zscore)){
    ...
    hist(xcoords, prob = TRUE, ylim = c(0,ymax), breaks = 30, xlim = c(min(mobs,min(xcoords,na.rm=TRUE),min(aux,na.rm=TRUE),na.rm=TRUE), max(mobs,max(xcoords,na.rm=TRUE),min(aux,na.rm=TRUE),na.rm=TRUE)), las = 1, col = "lightgray", border = "lightgray", xlab=xlab, ylab=ylab, main=paste(main, "p-value: ", pval, "\n Z-score: ", zscore, "\n n perm: ", nperm, "\n randomization: ", paste0(x$randomize.function.name)), cex.main=0.8, ...)
    
    if(plotType=="Area"){
      if(alternative=="greater"){
        polygon(c(aux,aux,max(mobs,max(xcoords,na.rm=TRUE),na.rm=TRUE),max(mobs,max(xcoords,na.rm=TRUE),na.rm=TRUE)),c(max(y,na.rm=TRUE),0,0,max(y,na.rm=TRUE)),col="red",density=10,border="white")
        lines(c(aux,aux),c(0,ymax*0.8),col="red",lwd=3)
        text(aux,ymax*0.9,bquote(alpha==.(pvalthres)),cex=0.8,pos=4)
      }
      if(alternative=="less"){
        polygon(c(aux,aux,min(mobs,min(xcoords,na.rm=TRUE),na.rm=TRUE),min(mobs,min(xcoords,na.rm=TRUE),na.rm=TRUE)),c(max(y,na.rm=TRUE),0,0,max(y,na.rm=TRUE)),col="red",density=10,border="white")
        lines(c(aux,aux),c(0,ymax*0.8),col="red",lwd=3)
    ...
    if(plotType=="Tailed"){
      if(alternative=="greater"){
        aux3<-seq(aux,max(max(xcoords,na.rm=TRUE),mobs,na.rm=TRUE),length=50)
      ...
      if(alternative=="less"){
        aux3<-seq(aux,min(min(xcoords,na.rm=TRUE),mobs,na.rm=TRUE),length=50)
        y3<-dnorm(aux3,mean(xcoords,na.rm=TRUE),sd(xcoords,na.rm=TRUE))
        polygon(c(aux3[1],aux3,aux3[length(aux3)]),c(0,y3,0),col="red",density=10,border="white")
        lines(c(aux,aux),c(0,ymax*0.8),col="red",lwd=3)
        text(aux,ymax*0.9,bquote(alpha==.(pvalthres)),cex=0.8)
